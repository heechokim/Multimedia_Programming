...
#define WIN_NAME "얼굴인식_김초희"
#define FACE_CLASSIFIER_PATH "default.xml"
...
//#define SOUND_NAME "warnning.wav"
...
int main(int argc, char *argv[]) {
	...
	if (!face_classifier.load("alt.xml")) {
			...
			try {
				...
				if (faces.size() != 0){
					...
					while ((index+1)%10!=0)
					{
						...
						//이미지로 저장  
						sprintf(buf, "c:/용의자/img_%06d.jpg", index);
					...
					//경고음 표시!!
					sndPlaySound("C:\\Temp\\53.wav", SND_LOOP);
					if ((index+1) % 10 == 0) index++;
